...
class paraProp:
    ...
    def __init__(self, iceLength, iceDepth, dx, dz, airHeight=25, filterDepth=100, refDepth=1):       
        ...
        #self.kz = np.fft.fftfreq(self.zNumFull)*2*np.pi
        
        # index of refraction array #
        self.n = np.ones(self.zNumFull)
        self.npr = np.ones(self.zNumFull)
        self.nprpr = np.ones(self.zNumFull)
    ...
    def set_n(self, nVal=None, nVec=None, nFunc=None, nAir=1.0003):
        ...
        ### ice profile functions ###
    def set_n(self, nVal=None, nVec=None, nFunc=None, nAir=1.0003, nprVal=None, nprVec=None, nprFunc=None, nprprVal=None, nprprVec=None, nprprFunc=None):
        """
        set the index of refraction profile and its derivatives for the simualtion
            - we use the sort hand that n is the index of refraction
            - np is the derivative of n with respect to z
            - npp is the second derivative of n with respect to z
        ...
        nVal : float
            Postcondition: n(z>=0, x>=0) = nVal
        npVal : float
            Postcondition: n(z>=0, x>=0) = npVal
        nppVal : float
            Postcondition: n(z>=0, x>=0) = nppVal
        nVec : array
            1-d or 2-d array of float values
            Precondition: spacing between rows is dz, spacing between columns is dx
            Postcondition: n(z=0,x=0) = nVec[0,0], n(z=dz,x=dx) = nVec[1,1], ..., n(z>=len(nVec[:,0])*dz,x>=len(nVec[0,:])*dx) = nVec[-1,-1]
        npVec : array
            1-d or 2-d array of float values
            Precondition: spacing between rows is dz, spacing between columns is dx
            Postcondition: np(z=0,x=0) = npVec[0,0], np(z=dz,x=dx) = npVec[1,1], ..., n(z>=len(npVec[:,0])*dz,x>=len(npVec[0,:])*dx) = npVec[-1,-1]
        nppVec : array
            1-d or 2-d array of float values
            Precondition: spacing between rows is dz, spacing between columns is dx
            Postcondition: npp(z=0,x=0) = nppVec[0,0], npp(z=dz,x=dx) = nppVec[1,1], ..., n(z>=len(nppVec[:,0])*dz,x>=len(nppVec[0,:])*dx) = nppVec[-1,-1]
        nFunc : function
            Precondition: nFunc is a function of one or two variables, z and x, and returns a float value
            Postcondition: n(z>=0,x>=0) = nFunc(z,x)
        npFunc : function
            Precondition: npFunc is a function of one or two variables, z and x, and returns a float value
            Postcondition: n(z>=0,x>=0) = npFunc(z,x)
        nAir : float
            index of refraction of air
            Postcondition: n(z<0) = nAir
        """    
        self.n = np.ones((self.zNumFull, self.xNum))
        self.npr = np.ones((self.zNumFull, self.xNum))
        self.nprpr = np.ones((self.zNumFull, self.xNum))
                        ...
                        for i in range(self.zNumFull):
                            for j in range(self.xNum):
                                if self.zFull[i] >= 0:
                                    ...
                                    self.n[i,j] = nAir
                    if len(nprVec.shape) == 1:
                        a = 0
                        nNum = len(nprVec)
                        for i in range(self.zNumFull):
                            if self.zFull[i] >= 0:
                                ai = a if a < nzNum else -1
                                self.npr[i,:] = nprVec[ai]
                                a += 1
                            else:
                                self.npr[i,:] = 0
                    elif len(nprVec.shape) == 2: 
                        a = 0
                        b = 0
                        nzNum = len(nprVec[:,0])
                        nxNum = len(nprVec[0,:])
                        for i in range(self.zNumFull):
                            for j in range(self.xNum):
                                if self.zFull[i] >= 0:
                                    ai = a if a < nzNum else -1
                                    bi = b if b < nxNum else -1
                                    self.npr[i,j] = nprVec[ai,bi]
                                    a += 1
                                    b += 1
                                else:
                                    self.npr[i,j] = 0
                    if len(nprprVec.shape) == 1:
                        a = 0
                        nNum = len(nprprVec)
                        for i in range(self.zNumFull):
                            if self.zFull[i] >= 0:
                                ai = a if a < nzNum else -1
                                self.nprpr[i,:] = nprprVec[ai]
                                a += 1
                            else:
                                self.nprpr[i,:] = 0
                    elif len(nprprVec.shape) == 2: 
                        a = 0
                        b = 0
                        nzNum = len(nprprVec[:,0])
                        nxNum = len(nprprVec[0,:])
                        for i in range(self.zNumFull):
                            for j in range(self.xNum):
                                if self.zFull[i] >= 0:
                                    ai = a if a < nzNum else -1
                                    bi = b if b < nxNum else -1
                                    self.nprpr[i,j] = nprprVec[ai,bi]
                                    a += 1
                                    b += 1
                                else:
                                    self.nprpr[i,j] = 0
                        ...
                        for i in range(self.zNumFull):
                            for j in range(self.xNum):
                                if self.zFull[i] >= 0:
                                    ...
                                    self.n[i,j] = nAir
                    sigpr = signature(nprFunc)
                    numParamspr = len(sigpr.parameters)
                    if numParamspr == 1:
                        for i in range(self.zNumFull):
                            if self.zFull[i] >= 0:
                                z = self.zFull[i] if self.zFull[i] <= self.iceDepth else self.iceDepth
                                self.npr[i,:] = nprFunc(z)
                            else:
                                self.npr[i,:] = 0
                    elif numParamspr == 2:
                        for i in range(self.zNumFull):
                            for j in range(self.xNum):
                                if self.zFull[i] >= 0:
                                    z = self.zFull[i] if self.zFull[i] <= self.iceDepth else self.iceDepth
                                    x = self.x[j]
                                    self.npr[i,j] = nprFunc(z,x)
                                else:
                                    self.npr[i,j] = 0
                    sigprpr = signature(nprprFunc)
                    numParamsprpr = len(sigprpr.parameters)
                    if numParamsprpr == 1:
                        for i in range(self.zNumFull):
                            if self.zFull[i] >= 0:
                                z = self.zFull[i] if self.zFull[i] <= self.iceDepth else self.iceDepth
                                self.nprpr[i,:] = nprprFunc(z)
                            else:
                                self.nprpr[i,:] = 0
                    elif numParamsprpr == 2:
                        for i in range(self.zNumFull):
                            for j in range(self.xNum):
                                if self.zFull[i] >= 0:
                                    z = self.zFull[i] if self.zFull[i] <= self.iceDepth else self.iceDepth
                                    x = self.x[j]
                                    self.nprpr[i,j] = nprprFunc(z,x)
                                else:
                                    self.nprpr[i,j] = 0
                else:
                    sig = signature(nFunc)
        ...
        self.n = np.transpose(self.n) 
        self.npr = np.transpose(self.npr)
        self.nprpr = np.transpose(self.nprpr)
    ...
    def get_npr(self):
        """
        gets derivative of the index of refraction profile of simulation
        
        Returns
        -------
        2-d float array
        """
        return np.transpose(self.npr[:,self.fNum1:-self.fNum2])
    ...
    def do_solver(self, rxList=np.array([])):
        ...
        for j in np.arange(0, int(self.freqNum/2)+self.freqNum%2, 1, dtype='int'):
            ...
            #Y = np.sqrt(1.+(((self.k0[j]*self.n*(self.zFull-self.refDepth))**2)*((self.npr*(self.zFull-self.refDepth)+self.n)**2)-1.*((self.npr**2)*((self.zFull-self.refDepth)**2)+self.n*self.nprpr*((self.zFull-self.refDepth)**2)+4*self.n*self.npr*(self.zFull-self.refDepth)+self.n**2)))   # Guassian derivative
            
            beta = np.exp(1.j * self.dx * self.k0[j] * (np.sqrt(B+Y**2)-Y))
            
            for i in range(1, self.xNum):               
                u = alpha * (util.doFFT(u))   ### Changed order of operations
